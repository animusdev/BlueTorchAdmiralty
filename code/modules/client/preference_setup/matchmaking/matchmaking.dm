...
/mob/living/Topic(href, href_list)
	...
	if(href_list["info_relation"])
		...
		if(istype(R))
			var/info = sanitize(input_utf8("What would you like the other party for this connection to know about your character?","Character info",R.info) as message|null)
			if(info)
				R.info = info
				see_relationship_info()
				return 1
