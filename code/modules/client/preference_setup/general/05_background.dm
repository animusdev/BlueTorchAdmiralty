...
/datum/category_item/player_setup_item/general/background/OnTopic(var/href,var/list/href_list, var/mob/user)
	if(href_list["nt_relation"])
		...
		var/new_medical = cp1251_to_utf8(sanitize(input(user,"Enter medical information here.","Character Preference", html_decode(edit_cp1251(pref.med_record))) as message|null, MAX_PAPER_MESSAGE_LEN, extra = 0))
		if(!isnull(new_medical) && !jobban_isbanned(user, "Records") && CanUseTopic(user))
			pref.med_record = post_edit_utf8(new_medical)
		return TOPIC_REFRESH

	else if(href_list["set_general_records"])
		var/new_general = cp1251_to_utf8(sanitize(input(user,"Enter employment information here.","Character Preference", html_decode(edit_cp1251(pref.gen_record))) as message|null, MAX_PAPER_MESSAGE_LEN, extra = 0))
		if(!isnull(new_general) && !jobban_isbanned(user, "Records") && CanUseTopic(user))
			pref.gen_record = post_edit_utf8(new_general)
		return TOPIC_REFRESH

	else if(href_list["set_security_records"])
		var/sec_medical = cp1251_to_utf8(sanitize(input(user,"Enter security information here.","Character Preference", html_decode(edit_cp1251(pref.sec_record))) as message|null, MAX_PAPER_MESSAGE_LEN, extra = 0))
		if(!isnull(sec_medical) && !jobban_isbanned(user, "Records") && CanUseTopic(user))
			pref.sec_record = post_edit_utf8(sec_medical)
		return TOPIC_REFRESH
