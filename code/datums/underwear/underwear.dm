...
/datum/category_item/underwear/proc/generate_image(var/list/metadata, var/mob/living/carbon/human/H)
	if(!icon_state)
		return
	var/image/I
	if(H && H.gender == FEMALE && H.species.name == "Human" || H.species.name == "Vat-Grown Human" || H.species.get_bodytype(H) == SPECIES_IPC || H.species.get_bodytype(H) == SPECIES_SKRELL)
		I = image(icon = 'icons/mob/slim/human.dmi', icon_state = icon_state)
	else
		I = image(icon = 'icons/mob/human.dmi', icon_state = icon_state)
