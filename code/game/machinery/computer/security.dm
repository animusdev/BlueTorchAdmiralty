...
//FIELD FUNCTIONS
			if ("Edit Field")
				...
				switch(href_list["field"])
					...
					if("notes")
						if (istype(active2, /datum/data/record))
							var/t1 = cp1251_to_utf8(sanitize(input("Please summarize notes:", "Secure. records", rhtml_decode(active2.fields["notes"]), null)  as message, extra = 0, max_length = MAX_PAPER_MESSAGE_LEN))
							if (!t1 || active2 != a2)
								return
