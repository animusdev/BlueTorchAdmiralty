...
/obj/item/proc/get_mob_overlay(mob/user_mob, slot)
	...
	if(icon_override)
		...
		if(ishuman(user_mob))
			var/mob/living/carbon/human/use_human = user_mob
			if(use_human.gender == FEMALE)
				if(use_human.species.name == "Human" || use_human.species.name == "Vat-Grown Human" || use_human.species.get_bodytype(use_human) == SPECIES_IPC || use_human.species.get_bodytype(use_human) == SPECIES_SKRELL)
